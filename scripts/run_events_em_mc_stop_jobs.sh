...
#------------------------------------------------------
# 2) Ejecutar ROOT llamando a em_loop_MC_opt_new_update()
#------------------------------------------------------
...
gROOT->ProcessLine("em_loop_MC_opt_new_update(\\"${CAMPAIGN}\\",${EVENT_ID},${STEPS},${TIME_GRID},${NDET},${MAX_STEPS},${BIN_SEED},\\"${CUT}\\",\\"${PHYLST}\\",\\"${SCF}\\",\\"${NEUFTY}\\")");
.q
EOF
